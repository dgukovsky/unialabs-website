...
const initFeatureWaves = () => {
  const cards = Array.from(document.querySelectorAll("[data-feature-wave-card]"));
  ...
  cards.forEach((card) => {
    const paths = Array.from(card.querySelectorAll("[data-wave]"));
    let frame = 0;
    let isVisible = true;
    let isActive = false;
    let targetX = 0.5;
    ...
    const drawWaves = (time = 0) => {
      ...
      if (isVisible && isActive && !prefersReducedMotion.matches) {
    ...
    const activate = () => {
      isActive = true;
      targetEnergy = 1;
      if (!frame && isVisible && !prefersReducedMotion.matches) {
        frame = window.requestAnimationFrame(drawWaves);
      }
    };
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          ...
          if (!isVisible && frame) {
            window.cancelAnimationFrame(frame);
            frame = 0;
          } else if (isVisible && isActive && !frame) {
            frame = window.requestAnimationFrame(drawWaves);
          }
        },
